...
class VariationalAutoencoder():
    ...
    def _build(self):
        ...
        for i in range(self.n_layers_decoder):
            ...
            if i < self.n_layers_decoder - 1:
                if self.use_batch_norm:
                    x = BatchNormalization()(conv_t_layer)
                x = LeakyReLU()(conv_t_layer)
                if self.use_dropout:
                    x = Dropout(rate=0.25)(x)
            else:
                x = Activation('sigmoid')(conv_t_layer)
    ...
    def train(self, x_train, batch_size, epochs, run_folder, print_every_n_batches=100,
        ...
        checkpoint_filepath = os.path.join(run_folder, "weights/weights.weights.h5")
